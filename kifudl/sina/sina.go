...
func (s *Sina) downloadKifu(sgf string) {
	...
	if err != nil {
		log.Println("sina - Could not parse kifu request:", err)
		return
	}
...
doRequest:
	...
	if err != nil {
		log.Println("sina - Could not send kifu request:", err)
		retry++
		if retry < 3 {
	...
	if resp.StatusCode != 200 {
		log.Println("sina - kifu request not 200")
		retry++
		if retry < 3 {
			time.Sleep(3 * time.Second)
			goto doRequest
	...
	if err != nil {
		log.Println("sina - cannot read kifu content", err)
		retry++
		if retry < 3 {
			time.Sleep(3 * time.Second)
			goto doRequest
...
func (s *Sina) downloadPage(page int) {
	...
	if err != nil {
		log.Println("sina - Could not parse page request:", err)
		return
	}
...
doPageRequest:
	resp, err := client.Do(req)
	if err != nil {
		log.Println("sina - Could not send page request:", err)
		retry++
		if retry < 3 {
			time.Sleep(3 * time.Second)
			goto doPageRequest
	...
	if err != nil {
		log.Println("sina - cannot read page content", err)
		retry++
		if retry < 3 {
			time.Sleep(3 * time.Second)
			goto doPageRequest
